...
def task4_main(path):
    ...
    checkpoint_file_triage.append("work_dirs/triage1600/epoch_19.pth") # 1st model
    checkpoint_file_triage.append("work_dirs/triage1600/epoch_11.pth") # 2nd model
    checkpoint_file_triage.append("work_dirs/triage_version2/epoch_51.pth") # 3rd model
    checkpoint_file_triage.append("work_dirs/triage1600/epoch_21.pth") # 4th model
    checkpoint_file_triage.append("work_dirs/valid_version1/epoch_17.pth") # 5th model

    # checkpoint_file_triage = "work_dirs/triage1600/epoch_19.pth"
    # ##################
    # # build the model from a config file and a checkpoint file
    model_person = init_detector(config_file_person, checkpoint_file_person, device="cuda:0")
    model_triage = []
    for i in range(5):
        model_triage.append(init_detector(config_file_triage, checkpoint_file_triage[i], device="cuda:0"))
    ...
    drone_keys = ["drone_1", "drone_2", "drone_3"]
    ensemble_answer = []
    for ensemble_num in range(len(model_triage)):
        person_results = []
        ...
        for set_n in range(1,6):
            ...
            for filename in glob.glob(set_dir): # filename : dataset_path/set_01/
                ...
                for file_idx in range(len(triage_list)):
                    ...
                    result_triage = inference_detector(model_triage[ensemble_num], ori_file) # person
                    labels = [
        ...
        print(task4_answer)
        ensemble_answer.append(task4_answer)


    
    final_answer = ensemble_answer[0]
...
# drone_keys = ["drone_1", "drone_2", "drone_3"]
    ...
    for s in range(5):
        for d in range(3): # drone
            per_drone_answer =[[] for model in range(len(ensemble_answer))]

            #for m in range(len(ensemble_answer)):
            #    per_drone_answer[i] = ensemble_answer[i][set_keys[s]][d]
            
        #for d in range(len(ensemble_answer[0][set_keys[s]])): # set_1 ~ set_5
            # ensemble_answer[0][set_keys[s]][0] : drone1
            # ensemble_answer[



# len(ensemble_answer) : 5
# len(model_traige) : 5

# ensemble_answer[i]['set_1'] => [{'drone_1': [{'set01_drone01_triage01': [0, 0, 0, 1], 'set01_drone01_triage02': [0, 0, 1, 0], 'set01_drone01_triage03': [0, 0, 1, 0]}]}, {'drone_2': [{'set01_drone02_triage01': [0, 1, 0, 1], 'set01_drone02_triage02': [0, 0, 1, 0]}]}, {'drone_3': [{'set01_drone03_triage01': [0, 1, 0, 0], 'set01_drone03_triage02': [0, 0, 1, 0]}]}]

# ensemble_answer[0] -> 0th model answer
# 

# ensemble_answer[i].keys() => dict_keys(['set_1', 'set_2', 'set_3', 'set_4', 'set_5'])
